...
2. **Release signing:** Keystores and passwords must be supplied through local or CI secrets and must never be committed.
...
- Release keystore and credentials are supplied through GitHub Actions secrets.
...
- Admin credentials must be managed through the server or a secret manager and must never be documented in the repository.
...
- **Django admin** (`/admin-django/`): Login with a Django staff superuser account
