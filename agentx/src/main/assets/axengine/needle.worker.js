...
var RUNTIME_VERSION = "ax-needle3-runtime-v1";
var BINARY_CACHE = "ax-needle3-assets-v1";
var WASM_URL = "./needle.wasm?v=1";
var CACT_URL = "./needle3.cact?v=1";
var SNAPSHOT_DB = "agentx-needle3";
...
function snapshotKey(tools) {
  return RUNTIME_VERSION + ":" + snapshotNamespace + ":" + hashText(systemPrompt + "|" + tools);
}
...
async function idbDelete(key) {
  ...
  return new Promise(function (resolve, reject) {
    var transaction = db.transaction(SNAPSHOT_STORE, "readwrite");
    var request = transaction.objectStore(SNAPSHOT_STORE).delete(key);
...
async function idbDeleteExcept(prefix) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    transaction.onerror = function () {
      db.close();
      reject(new Error("IndexedDB cleanup failed"));
    };
  });
}
...
async function restoreSnapshot(runtime, tools) {
  ...
  if (!metadata || metadata.version !== RUNTIME_VERSION ||
      metadata.byteLength < 16 * 1024 * 1024 || metadata.byteLength > 160 * 1024 * 1024 ||
...
function initializeTools(runtime, tools) {
  ...
  JSON.parse(tools);
  var promptPointer = allocateCString(runtime, systemPrompt);
...
async function ensureRuntime(tools, options) {
  ...
  if (options) {
    ...
    if (typeof options.system === "string") systemPrompt = options.system;
...
async function embed(message) {
  var runtime = await ensureRuntime(message.tools, message);
  var textPointer = allocateCString(runtime, message.text || "");
  try {
    var dim = runtime._needle_embed(textPointer, 0, 0);
    if (!(dim > 0) || dim > 4096) throw new Error("embedding failed");
    var outPointer = runtime._malloc(dim * 4);
    try {
      var rc = runtime._needle_embed(textPointer, outPointer, dim);
      if (rc < 0) throw new Error("embedding failed");
      var view = new Float32Array(runtime.HEAPU8.buffer, outPointer, dim);
      self.postMessage({ type: "embedding", id: message.id, values: Array.from(view) });
    } finally {
      runtime._free(outPointer);
    }
  } finally {
    runtime._free(textPointer);
    try { runtime._needle_reset(); } catch (_) { }
  }
}
...
self.addEventListener("message", function (event) {
  ...
  if (message.type === "embed") {
    embed(message).catch(function (error) { postError(error, message.id); });
    return;
  }
