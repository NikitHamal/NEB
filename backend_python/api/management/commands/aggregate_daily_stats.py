...
def aggregate_date(target_date, force=False):
    """Aggregate a single date into a DailyStat record. Returns True if created."""
    import time as _time
    day_start_ms = int(_time.mktime(target_date.timetuple()) * 1000)
    ...
    if total_sessions > 0:
        durations = []
        for key_info in session_data[:50]:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(days):
            ...
            if aggregate_date(target_date, force=redo_all):
                aggregated += 1
