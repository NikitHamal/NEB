...
def arena_k2think_models(request):
    models = k2think_proxy.get_models()
    return Response({'models': models, 'provider': 'k2think'})


@api_view(['POST'])
@authentication_classes([AuthTokenAuthentication])
@throttle_classes([ArenaListRateThrottle, ArenaChatRateThrottle])
def arena_create_k2think_session(request):
    ...
    sess = ArenaChatSession.objects.create(
        ...
        user=user,
        provider='k2think',
        arena_session_id=f'k2think:{_secrets.token_hex(16)}',
        arena_token_id='',
        qwen_chat_id='',
        model_id=model_id,
        model_code=model_id,
...
def arena_send_message_k2think(request, session_id):
    user, err = _require_user(request)
    if err:
        return err
    ...
    def _stream_k2think():
        ...
        try:
            for chunk in k2think_proxy.stream_chat(
                ...
                if t == 'text':
                    ...
                    logger.warning("k2think stream: %s", error_text)
                    yield _sse_format({
                        'error': {'message': error_text, 'code': 'upstream'},
                    })
                    break
        except Exception as e:
            logger.exception("k2think stream: %s", e)
            yield _sse_format({
        ...
        try:
            ...
            logger.exception("k2think: failed to finalize assistant row: %s", e)
        ...
        if not error_text:
            try:
                ...
                logger.warning("k2think: counter commit failed: %s", e)
    ...
    response = StreamingHttpResponse(_stream_k2think(), content_type='text/event-stream')
