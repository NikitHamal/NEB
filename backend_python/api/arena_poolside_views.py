"""Arena views for Poolside (chat.poolside.ai/guest — Laguna S 2.1 / XS 2.1)."""
import json
import logging
...
def arena_poolside_models(request):
    models = poolside_proxy.get_models()
    return Response({'models': models, 'provider': 'poolside'})


@api_view(['POST'])
@authentication_classes([AuthTokenAuthentication])
@throttle_classes([ArenaListRateThrottle, ArenaChatRateThrottle])
def arena_create_poolside_session(request):
    user, err = _require_user(request)
    if err:
        return err

    model_id = (request.data.get('modelId') or 'laguna-s-2.1').strip()

    model_info = poolside_proxy.MODEL_MAP.get(model_id)
    display_name = (model_info or {}).get('name', f'Laguna {model_id}')
    ...
    sess = ArenaChatSession.objects.create(
        ...
        user=user,
        provider='poolside',
        arena_session_id=f'poolside:{_secrets.token_hex(16)}',
        arena_token_id='',
        qwen_chat_id='',
        model_id=model_id,
        model_code=model_id,
...
def arena_send_message_poolside(request, session_id):
    user, err = _require_user(request)
    if err:
        return err
    ...
    if sess.provider != 'poolside':
        return Response(
            {'error': 'This endpoint only works with Poolside sessions.'},
    ...
    def _stream_poolside():
        ...
        try:
            for chunk in poolside_proxy.stream_chat(
                ...
                if t == 'text':
                    ...
                    logger.warning("poolside stream: %s", error_text)
                    yield _sse_format({
                        'error': {'message': error_text, 'code': 'upstream'},
                    })
                    break
        except Exception as e:
            logger.exception("poolside stream: %s", e)
            yield _sse_format({
        ...
        try:
            ...
            logger.exception("poolside: failed to finalize assistant row: %s", e)
        ...
        if not error_text:
            try:
                ...
                logger.warning("poolside: counter commit failed: %s", e)
    ...
    response = StreamingHttpResponse(_stream_poolside(), content_type='text/event-stream')
