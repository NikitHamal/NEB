"""Arena views for Inception Labs (Mercury 2 diffusion LLM)."""
import json
import logging
...
def arena_inception_models(request):
    models = inception_proxy.get_models()
    return Response({'models': models, 'provider': 'inception'})


@api_view(['POST'])
@authentication_classes([AuthTokenAuthentication])
@throttle_classes([ArenaListRateThrottle, ArenaChatRateThrottle])
def arena_create_inception_session(request):
    user, err = _require_user(request)
    if err:
        return err

    model_id = (request.data.get('modelId') or 'mercury-2').strip()

    model_info = inception_proxy.MODEL_MAP.get(model_id)
    display_name = (model_info or {}).get('name', f'Inception {model_id}')
    ...
    sess = ArenaChatSession.objects.create(
        ...
        user=user,
        provider='inception',
        arena_session_id=f'inception:{_secrets.token_hex(16)}',
        arena_token_id='',
        qwen_chat_id='',
        model_id=model_id,
        model_code=model_id,
...
def arena_send_message_inception(request, session_id):
    user, err = _require_user(request)
    if err:
        return err
    ...
    if sess.provider != 'inception':
        return Response(
            {'error': 'This endpoint only works with Inception sessions.'},
    ...
    reasoning_effort = request.data.get('reasoningEffort', 'high')

    def _stream_inception():
        yield _sse_format({
            'choices': [{
                'index': 0,
        ...
        try:
            for chunk in inception_proxy.stream_chat(
                ...
                if t == 'text':
                    ...
                    logger.warning("inception stream: %s", error_text)
                    yield _sse_format({
                        'error': {'message': error_text, 'code': 'upstream'},
                    })
                    break
        except Exception as e:
            logger.exception("inception stream: %s", e)
            yield _sse_format({
        ...
        try:
            ...
            logger.exception("inception: failed to finalize assistant row: %s", e)
        ...
        if not error_text:
            try:
                ...
                logger.warning("inception: counter commit failed: %s", e)
    ...
    response = StreamingHttpResponse(_stream_inception(), content_type='text/event-stream')
